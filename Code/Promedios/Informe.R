...
informe_data <- eventReactive(input$Informe_boton,{
  
  # INPUTS INFORME ----------------
  facultad_aux <- input$facultad_in
  carrera_aux <- input$carrera_in
  period_aux  <- input$periodo_in
  n_periodos_aux <- as.numeric(input$n_periodo_in)
  ...
  porcentaje_rprb_hist$Reprob_Media <- apply(X=porcentaje_rprb_hist[,c(-1,-2)],MARGIN=1,FUN=mean_na)
  ...
  Reprobados_fit <- data.frame("Materia"=reprobados_hist$Materia, "Reprobados Prediccion"=
                                 round(as.numeric(porcentaje_rprb_hist$Reprob_Media)*as.numeric(matriculad_aux[,2])))
  names(Reprobados_fit) <- c("Materia","Reprobados Prediccion")
  ...
  return(list('informe'=informe,'tabla_historico'=tabla_historico))
})

#Titulo de Informe MACRO -------------------------------------
output$titulo_macro <- renderText({ 
  paste("Informe de Reprobación MACRO")
})
output$facultad_macro <- renderText({ 
  paste("Facultad: ", input$facultad_in)
})
output$carrera_macro <- renderText({ 
  carrera_aux = input$carrera_in
...
output$periodos_macro <- renderText(expr={
  # INPUTS  ------- 
  facultad_aux <- input$facultad_in
  carrera_aux <- input$carrera_in
  period_aux  <- input$periodo_in
...
output$informe_reprob<-renderDataTable(server = FALSE,
                                       expr={
